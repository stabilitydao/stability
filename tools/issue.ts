import {
  baseStrategyContracts, ChainName,
  getStrategyProtocols, integrations,
  strategies,
  StrategyShortId,
  StrategyState,
...
for (const shortId of Object.keys(strategies)) {
  ...
  if (strategy.contractGithubId === 'is-being-created') {
    console.log(`------------------ Need to create or update issue for strategy ${strategy.shortId}`)
    ...
    for (const protocol of protocols) {
      ...
      console.log(`<a href="${org.website}" target="_blank"><img src="${img}" width="80px" height="80px" alt="${protocol.name}"></a>`)
    }
    console.log('</div>')
    console.log('')

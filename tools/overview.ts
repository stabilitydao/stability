...
console.log(`Contests: ${Object.keys(contests).filter(c => !contests[c].hidden).length}. Banner images: ${Object.keys(contests).filter(c => !contests[c].hidden).filter(c => !!contests[c].img).length}, quest platform integrations: ${Object.keys(contests).filter(c => !contests[c].hidden).filter(c => !!contests[c].integration).length}.`)
console.log(``)
